...
import { Activity, Eye, EyeOff, ArrowRight, Building2, Stethoscope, Bed, Pill, CreditCard, Truck, Microscope, ShieldCheck, CheckCircle2, UserCheck } from 'lucide-react';
...
const departments = [
  {
    ...
    user: { first_name: 'Saikiran', last_name: 'Reddy', role: 'ADMIN', department: 'Administration', title: 'Chief Administrator' },
    icon: Building2,
    color: '#0284c7',
    bg: '#e0f2fe',
  ...
  },
  {
    ...
    user: { first_name: 'Priya', last_name: 'Nair', role: 'DOCTOR', department: 'Cardiology', title: 'Senior Cardiologist' },
    icon: Stethoscope,
    color: '#0d9488',
    bg: '#ccfbf1',
  ...
  },
  {
    ...
    user: { first_name: 'Ramesh', last_name: 'Kumar', role: 'PHARMACIST', department: 'Pharmacy', title: 'Chief Pharmacist' },
    icon: Pill,
    color: '#d97706',
    bg: '#fef3c7',
  ...
  },
  {
    ...
    user: { first_name: 'Sunita', last_name: 'Rao', role: 'FINANCE', department: 'Billing & Accounts', title: 'Finance Manager' },
    icon: CreditCard,
    color: '#4f46e5',
    bg: '#e0e7ff',
  ...
  },
  {
    ...
    user: { first_name: 'Raju', last_name: 'Sharma', role: 'DISPATCHER', department: 'Emergency & Transport', title: 'Fleet Controller' },
    icon: Truck,
    color: '#ea580c',
    bg: '#ffedd5',
  ...
  },
  {
    ...
    user: { first_name: 'Amit', last_name: 'Shah', role: 'LAB_TECH', department: 'Diagnostics & ML', title: 'Lab Director' },
    icon: Microscope,
    color: '#8b5cf6',
    bg: '#ede9fe',
...
export const Login = () => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', width: '100%', fontFamily: 'Inter, system-ui, sans-serif', background: '#f8fafc' }}>
      {/* Left Branding Panel */}
      <div style={{ width: '42%', padding: '3.5rem 3rem', background: 'linear-gradient(135deg, #0284c7 0%, #0369a1 50%, #0f172a 100%)', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
        {/* Glow Effects */}
        ...
        <div style={{ position: 'relative', zIndex: 1, my: 'auto' }}>
          <h2 style={{ fontSize: '2.5rem', fontWeight: 900, lineHeight: 1.2, marginBottom: '1.25rem', letterSpacing: '-0.02em' }}>
            Departmental Staff <br />
            <span style={{ color: '#38bdf8' }}>Access Portal</span>
          </h2>
          <p style={{ color: '#bae6fd', lineHeight: 1.7, fontSize: '1rem', maxWidth: 440, marginBottom: '2rem' }}>
            Select your assigned department to unlock role-tailored clinical dashboards, ICU vitals telemetry, patient queues, and pharmacy inventory.
          </p>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'rgba(255,255,255,0.08)', padding: '0.75rem 1rem', borderRadius: 10, backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.12)' }}>
              <CheckCircle2 size={20} color="#38bdf8" />
              <span style={{ fontSize: '0.875rem', fontWeight: 600 }}>Role-Based Access Control (RBAC) Enforced</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'rgba(255,255,255,0.08)', padding: '0.75rem 1rem', borderRadius: 10, backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.12)' }}>
              <ShieldCheck size={20} color="#34d399" />
              <span style={{ fontSize: '0.875rem', fontWeight: 600 }}>HIPAA Audit Ledger & End-to-End Encryption</span>
            </div>
          </div>
        </div>

        <p style={{ color: '#7dd3fc', fontSize: '0.85rem', position: 'relative', zIndex: 1 }}>
          &copy; 2026 HealthSphere Enterprise Inc. All rights reserved.
        </p>
      </div>
        ...
        <div style={{ maxWidth: 660, margin: '0 auto', width: '100%' }}>
          ...
          <div style={{ marginBottom: '1.75rem' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.95rem' }}>
              Click any department card below for <strong>One-Click Instant Demo Login</strong>
            </p>
          </div>

          {/* Department Quick Selection Grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: '0.85rem', marginBottom: '1.75rem' }}>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: isSelected ? '0 4px 14px rgba(0,0,0,0.08)' : '0 1px 3px rgba(0,0,0,0.04)'
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: '0.875rem', fontWeight: 800, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {dept.name.split(' ')[0]}
                    </span>
                  </div>
          ...
          <div style={{ padding: '1rem 1.25rem', background: selectedDept.bg, border: `1.5px solid ${selectedDept.color}`, borderRadius: 12, marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContext: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.825rem', color: '#475569', marginTop: 2 }}>
                  <strong>{selectedDept.user.first_name} {selectedDept.user.last_name}</strong> &bull; {selectedDept.user.title}
                </div>
              </div>
            </div>
            <span className="badge" style={{ background: selectedDept.color, color: 'white', marginLeft: 'auto', padding: '0.35rem 0.75rem' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.875rem' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: '#64748b', fontWeight: 500 }}>
                <input type="checkbox" defaultChecked style={{ accentColor: selectedDept.color }} /> Remember this department
              </label>
              <a href="#" style={{ color: selectedDept.color, fontWeight: 700, textDecoration: 'none' }}>Forgot password?</a>
            </div>
            ...
            <button 
              ...
              disabled={loading} 
              style={{ background: selectedDept.color, color: 'white', padding: '0.9rem', fontSize: '1rem', borderRadius: 10, fontWeight: 800, boxShadow: `0 4px 14px ${selectedDept.color}40` }}
            >
              {loading ? <div className="spinner-small"></div> : <><span>Sign In to {selectedDept.name.split(' ')[0]}</span> <ArrowRight size={18} /></>}
            </button>
          </form>
